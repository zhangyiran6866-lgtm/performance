  ...
  <div class="h-screen flex flex-col bg-slate-50/50 overflow-hidden">
        ...
        <div class="flex items-center gap-6">
          <div class="flex items-center gap-3">
            <div class="bg-indigo-600 p-3 rounded-2xl shadow-lg shadow-indigo-100 flex items-center justify-center">
               <el-icon class="text-2xl text-white"><LayoutDashboard /></el-icon>
            </div>
            <div>
               <h1 class="text-2xl font-black text-slate-900 leading-none">绩效工作台</h1>
               <p class="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">Performance Hall</p>
            </div>
          </div>
          ...
          <div class="hidden lg:flex items-center gap-5 bg-slate-50 px-5 py-2.5 rounded-2xl border border-slate-100">
             <el-icon class="text-slate-400 text-lg"><Calendar /></el-icon>
             ...
             <el-tag :color="phaseInfo.color" effect="dark" class="border-none px-3 font-bold text-xs ml-2 custom-phase-tag">
                {{ phaseInfo.label }}
             </el-tag>
          </div>
        </div>
        ...
        <div class="flex items-center gap-3">
          <template v-if="activeTab === 'team_rating' && !isRatingLocked">
             <div class="flex items-center gap-6 mr-1">
               ...
               <el-button type="primary" class="custom-submit-btn">
                  全部提交
               </el-button>
             </div>
          </template>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <el-tab-pane name="my_performance">
            <template #label>
              <div class="flex items-center gap-2 h-14">
                <el-icon :size="20"><UserCircle /></el-icon>
                <span>我的绩效</span>
              </div>
            </template>
          </el-tab-pane>
          
          <el-tab-pane v-if="userRole === 'manager'" name="team_goals">
            <template #label>
              <div class="flex items-center gap-2 h-14 relative group">
                <el-icon :size="20" :class="isGoalSettingLocked ? 'text-slate-300' : 'text-slate-400 group-[.is-active]:text-indigo-600'">
                  <Target />
                </el-icon>
                <span :class="isGoalSettingLocked ? 'text-slate-300' : 'text-slate-500 group-[.is-active]:text-indigo-600'">团队目标设定</span>
                <el-icon v-if="isGoalSettingLocked" class="ml-1 text-slate-300"><Lock /></el-icon>
              </div>
            </template>
          </el-tab-pane>

          <el-tab-pane v-if="userRole === 'manager'" name="team_rating">
            <template #label>
              <div class="flex items-center gap-2 h-14 relative group">
                <el-icon :size="20" :class="isRatingLocked ? 'text-slate-300' : 'text-slate-400 group-[.is-active]:text-indigo-600'">
                  <Award />
                </el-icon>
                <span :class="isRatingLocked ? 'text-slate-300' : 'text-slate-500 group-[.is-active]:text-indigo-600'">团队绩效评价</span>
                <el-icon v-if="isRatingLocked" class="ml-1 text-slate-300"><Lock /></el-icon>
              </div>
            </template>
          </el-tab-pane>
        </el-tabs>
      ...
      <div class="flex-1 overflow-hidden relative">
        ...
        <TeamGoalWorkspace v-else-if="activeTab === 'team_goals'" :is-locked="isGoalSettingLocked" />
        <RatingWorkspace v-else-if="activeTab === 'team_rating'" :is-locked="isRatingLocked" />
